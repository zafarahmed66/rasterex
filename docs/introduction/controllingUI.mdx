...
The `config.json` file provides a static configuration for the Viewer, enabling customization of its UI and functionality.
...
The parameters, which are distinct from defaultconfig of the Rasterex itself and that allow to parameterize the UI of the viewer are as listed below.


### Logo Configuration
`<LogoUrlX>/assets/images/logo.svg</LogoUrlX>`

For the ease of the branding in case the need of setting up a customer-tailored version the logo and color palette configs can be set in the **defaultconfig.xml**.

The image used as a logo that is presented in the viewer (e.g. in the header of the viewspace) can be set with the help of the below parameter. `<LogoUrlX>/assets/images/logo.svg</LogoUrlX>`

---

### Color Palette

The color palette is defined with the help of the tags listed below.
...
- `<GuiColorPaletteX>`
  ...
  - `<light>#F0F7F9</light>`  
    A light shade used for backgrounds or accents to create contrast or emphasis.  
